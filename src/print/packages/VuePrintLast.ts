...
const FUNC_NAME = "[VuePrintLast]";

export default class VuePrintLast {
  // html 文档标准
  ...
  private init() {
    ...
    throw new Error(
      `${FUNC_NAME}: Either "el"、"url" or "asyncUrl" parameter must be provided in the settings.`
    );
  }
  ...
  private previewIframeLoad() {
    const box = document.getElementById("vue-pirnt-last-previewBox");
    if (!box) return;
  ...
  private getNoPrintMediaStyle(): string {
    ...
    if (!isArray && !isString) {
      console.error(
        new TypeError(
          `${FUNC_NAME}: The "noPrintSelector" must be either a string or an array of strings. Please check your settings.`
        )
      );
      return "";
    }
    const noPrintSelectorList = Array.isArray(noPrintSelector)
      ? noPrintSelector
      : [noPrintSelector];
    const selectorStr = noPrintSelectorList
  ...
  private getPrintMediaStyle() {
    const customSizeStyle = this.settings.customSize
      ? `@page { size: ${this.settings.customSize.width} ${this.settings.customSize.height}; }`
      : "";
    return `${customSizeStyle}`;
  }
  ...
  private getHead(): string {
    ...
    return `<head>
	 ...
	 ${extraHead}${links}
	 <style type="text/css">${style}${printMediaStyle}${noPrintMediaStyle}</style>
	 ${extraCss}
 </head>`;
  }
  ...
  getPrintAreaDom(): HTMLElement[] {
    ...
    if (!isSelector) {
      throw new TypeError(
        `${FUNC_NAME}: The "el" property should be either a string (CSS selector) or an HTMLElement, but received type "${typeof el}".`
      );
    }
    ...
    if (!contentDom?.length) {
      throw new Error(
        `${FUNC_NAME}: No elements found matching the selector: "${el}".`
      );
    }
    ...
    copiedInputs.forEach((item) => {
      ...
      switch (typeInput) {
        case "select":
          selectCount++;
          const select =
            originalElement.querySelectorAll<HTMLSelectElement>("select")[
              selectCount
            ];
          if (select) {
            const opSelectedIndex = select.selectedIndex;
            (item as HTMLSelectElement).options[opSelectedIndex].setAttribute(
              "selected",
              "selected"
            );
          }
  ...
  private getPrintWindow(url: string): PrintAreaWindow {
    ...
    if (!doc) {
      throw new Error(
        `${FUNC_NAME}: Unable to find the document object within the created iframe. Please ensure the iframe is correctly created and loaded.`
      );
    }
  ...
  private previewBox(): {
    ...
    let box = document.getElementById("vue-pirnt-last-previewBox");
    if (box) {
      box.querySelector("iframe")?.remove();
      return {
        close: box.querySelector(".previewClose"),
        previewBody: box.querySelector(".previewBody")
      };
    }

    // 创建预览框架
    box = document.createElement("div");
    box.setAttribute("id", "vue-pirnt-last-previewBox");
    box.setAttribute(
      "style",
      "position: fixed; top: 0px; left: 0px; width: 100%; height: 100%; background: white; display: none; z-index: " +
  ...
  private createCloseButton(): HTMLElement {
    ...
    const closeStyles =
      "position: absolute; width: 3px; height: 100%; background: #040404; top: 0px; left: 50%;";
